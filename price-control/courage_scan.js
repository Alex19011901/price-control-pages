...
const OUT = process.argv[2] || path.join(__dirname, 'courage_audit.json');
...
function isCourage(name){const n=norm(name);return n.includes('огур')&&n.includes('кураж');}
...
(async()=>{
  ...
  async function readInvoice(page,task){
    ...
    for(const item of detail.items||[]){
      if(!isCourage(item.name))continue;
      candidates.push({date:supply.date,supplier:supply.supplier,invoice:doc.invoice,publicId:inv.publicId,version,line:item.line,name:item.name,unit:item.unit,qty:item.count,sum:item.sum,fact:item.count?Math.round((item.sum/item.count+Number.EPSILON)*100)/100:null});
    }
  }
  ...
  const payload={
    generatedAt:new Date().toISOString(),start:'01.01.2026',suppliesScanned:supplies.length,invoicesScanned:scanned,invoiceErrors:errors.length,errors,
    summary:{label:'Огурцы Кураж',rows:active.length,invoices:new Set(active.map(x=>x.invoice)).size,exactNames:[...new Set(active.map(x=>`${x.name} | ${x.unit}`))].sort((a,b)=>a.localeCompare(b,'ru')),suppliers:[...new Set(active.map(x=>x.supplier))].sort((a,b)=>a.localeCompare(b,'ru')),firstDate:active[0]?.date||null,lastDate:active[active.length-1]?.date||null},
    matches:active
  };
